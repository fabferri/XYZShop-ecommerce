# XYZShop — Database Schema

Entity–Relationship diagram of the XYZShop database, rendered natively by GitHub
via [Mermaid](https://mermaid.js.org/). Generated from the Django models in
[`products/models.py`](products/models.py) and [`orders/models.py`](orders/models.py)
(plus Django's built-in `auth_user`).

> A raster version is also available at [`db-schema.jpg`](db-schema.jpg)
> (regenerate with `python generate_db_schema.py`).

## Legend

- **PK** — primary key · **FK** — foreign key · **UK** — unique key
- `||--o{` — one-to-many (mandatory parent) · `|o--o{` — one-to-many (optional/nullable parent)
...
- **`PRODUCT_REVIEW`** has a composite unique constraint `UNIQUE(product_id, user_id)`
  — one review per user per product.
- **Cascade behaviour**: all FKs are `ON DELETE CASCADE` **except**
  `PRODUCT_PRICE_HISTORY.changed_by_id` which is `ON DELETE SET NULL`.
- **Nullable FKs**: `ORDER.user_id` (guest orders), `SALE.order_id`,
  and `PRODUCT_PRICE_HISTORY.changed_by_id`.
- **Signals**: `PRODUCT_PRICE_HISTORY` rows are created automatically when a
  product's price/cost changes; `SALE` rows are created automatically when an
  `ORDER` is marked `paid`.
- The **shopping cart is session-based** (`request.session['cart']`) and has **no
  database table**, so it does not appear in this schema.
